...
def build_gene_list(cna_table, genelist, base_genes='default'):
    if base_genes=='default':
        genes=["PPM1D", "TP53",  "BRCA1", "BRCA2", "MECOM", "RB1", "PTEN", "PALB2","ERBB2", "CDK12", "PIK3CA", "KRAS", "CCNE1", "MYC"]
    # additional_genes = pd.read_csv(cna_table, sep="\t")["Gene"].unique().tolist()
    # genes = set(genes + additional_genes)
...
def generate_gistic_outputs(gistic_data, hdel_data, cbio_table):
    ...
    gistic_matrix.to_csv("test", sep="\t", index=False, na_rep="NA")
...
def _write_maf(m, label, merged_maf, write_header):
    ...
    for chunk in maf:
        ...
        chunk= chunk.astype({"t_ref_count":"Int64", "t_alt_count":"Int64", "n_ref_count":"Int64", "n_alt_count":"Int64"})
